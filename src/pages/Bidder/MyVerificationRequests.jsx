...
const MyVerificationRequests = () => {
    ...
    try {
      ...
      if (silent) {
        ...
        if (newlyApproved.length > 0) {
          toast.success(`${newlyApproved.length} request(s) approved! Check your email for the code.`, {
            duration: 5000,
            icon: '🎉',
          });
        }
      ...
      case 'pending':
        return (
          <motion.span
            ...
            className="inline-flex items-center space-x-1 px-2 sm:px-3 py-1 sm:py-1.5 bg-yellow-500/20 text-yellow-400 text-xs sm:text-sm rounded-full border border-yellow-400/30"
          >
  ...
  const filteredRequests = requests.filter(request => {
    const matchesSearch = 
      ...
      request.tender?.category?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getStatusMessage = (request) => {
    ...
    switch (status) {
      ...
      case 'rejected':
        return request.rejectionReason || "Your request was not approved. Please contact the tender issuer for more information.";
      case 'used':
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        <motion.div
          ...
          className="bg-white/5 backdrop-blur-xl border border-cyan-400/20 rounded-lg sm:rounded-xl p-3 sm:p-4 hover:bg-white/10 transition-all duration-300"
        >
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-yellow-400 mt-0.5 sm:mt-1">{stats.pending}</p>
            </div>
          ...
          {stats.pending > 0 && (
            <p className="text-[10px] sm:text-xs text-gray-500 mt-1 sm:mt-2">Auto-refresh...</p>
          )}
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-green-400 mt-0.5 sm:mt-1">{stats.approved}</p>
            </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl sm:text-2xl font-bold text-red-400 mt-0.5 sm:mt-1">{stats.rejected}</p>
            </div>
      ...
      <div className={`${showMobileFilters ? 'block' : 'hidden'} sm:block space-y-3 sm:space-y-0 sm:flex sm:flex-row sm:gap-4 mb-4 sm:mb-6`}>
          ...
          <AnimatePresence>
            {filteredRequests.map((request, index) => (
              ...
              >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-400 mb-2 sm:mb-3">
                            ...
                            <span className="flex items-center space-x-1">
                              ...
                              <span className="sm:hidden">{formatDateMobile(request.createdAt)}</span>
                            </span>
                              ...
                              <div className="flex-1">
                                ...
                                {request.status === 'approved' && request.code && (
                                  <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-slate-900/50 border border-cyan-400/30 rounded-lg">
                                    <p className="text-xs sm:text-sm text-gray-400 mb-2 text-center">Your Verification Code</p>
                                    <div className="flex flex-col sm:flex-row items-center sm:justify-between gap-3">
                                      ...
                                      <button
                                        onClick={() => handleCopyCode(request.code)}
                                      ...
                                      >
                                        ...
                                        <span className="sm:hidden text-sm">Copy Code</span>
                                      </button>
                                    </div>
                                  </div>
                                )}
                          ...
                          <div className="mt-3 sm:mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 text-xs sm:text-sm">
                            <div>
                              <p className="text-gray-500 mb-0.5 sm:mb-1">Request ID</p>
                            ...
                            {request.approvedAt && (
                              <div>
                                <p className="text-gray-500 mb-0.5 sm:mb-1">Approved</p>
                                <p className="text-gray-300 truncate">
                                  <span className="hidden sm:inline">{formatDate(request.approvedAt)}</span>
                                  <span className="sm:hidden">{formatDateMobile(request.approvedAt)}</span>
                                </p>
                              </div>
                            )}
                            {request.rejectedAt && (
                              <div>
                                <p className="text-gray-500 mb-0.5 sm:mb-1">Rejected</p>
                                <p className="text-gray-300 truncate">
                                  <span className="hidden sm:inline">{formatDate(request.rejectedAt)}</span>
                                  <span className="sm:hidden">{formatDateMobile(request.rejectedAt)}</span>
                                </p>
                              </div>
                            )}
                            {request.expiresAt && (
                              <div>
                                <p className="text-gray-500 mb-0.5 sm:mb-1">Expires</p>
                                <p className="text-gray-300 truncate">
                                  <span className="hidden sm:inline">{formatDate(request.expiresAt)}</span>
                                  <span className="sm:hidden">{formatDateMobile(request.expiresAt)}</span>
                                </p>
                              </div>
                            )}
                          </div>
                ...
                >
                  <span className="text-xs sm:text-sm text-gray-400">View Tender Details</span>
                ...
                <AnimatePresence>
                  {expandedRequest === request._id && (
                    <motion.div
                      ...
                      className="px-4 sm:px-6 pb-4 sm:pb-6 border-t border-cyan-400/10"
                    >
                      <div className="pt-3 sm:pt-4 space-y-3">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
                          ...
                          <div>
                            <p className="text-gray-500 mb-0.5 sm:mb-1">Budget Range</p>
                            <p className="text-cyan-400 truncate">
                              R{request.tender?.budgetMin?.toLocaleString() || 0} - R{request.tender?.budgetMax?.toLocaleString() || 0}
                            </p>
                          </div>
                          <div>
                            <p className="text-gray-500 mb-0.5 sm:mb-1">Deadline</p>
                            <p className="text-gray-300">
                              {request.tender?.deadline ? formatDateMobile(request.tender.deadline) : 'N/A'}
                            </p>
                          </div>
                          <div>
                        ...
                        {request.tender?.description && (
                          <div>
                            <p className="text-gray-500 mb-0.5 sm:mb-1 text-xs sm:text-sm">Description</p>
      ...
      <motion.div
        ...
        className="mt-6 sm:mt-8 p-4 sm:p-6 bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border border-cyan-400/20 rounded-lg sm:rounded-xl"
      >
