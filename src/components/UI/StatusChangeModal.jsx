...
const StatusChangeModal = ({ application, action, onClose, onSubmit }) => {
  ...
  return createPortal(
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-[10050]"
      onClick={() => {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-white">{action} Application</h3>
          <button
            onClick={() => {
              if (!submitting) onClose();
            }}
